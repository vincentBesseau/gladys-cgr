...
import { checkForNewFilms } from '../src/cgr/newFilmPolling.js';
...
const movieB = {
  ...
  sourceUrl: 'https://www.cgrcinemas.fr/films-a-l-affiche/2-b/',
...
async function setup() {
  dir = await mkdtemp(join(tmpdir(), 'cgr-new-film-'));
...
test('a fetch failure keeps the previous baseline instead of throwing', async () => {
  ...
  await checkForNewFilms(
    gladys,
    async () => {
      throw new Error('cgrcinemas.fr unreachable');
    },
    state,
  );
...
test('a publishSceneEvent failure for one film does not stop the others from firing', async () => {
  ...
  const movieC = {
    ...
    sourceUrl: 'https://www.cgrcinemas.fr/films-a-l-affiche/3-c/',
  };
  const state = buildState();
