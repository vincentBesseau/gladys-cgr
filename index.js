...
// for the films currently playing at ONE configured CGR cinema. It also
// exposes a "Find my cinema" action to look up the cinema's ID.
//
...
import { buildNowPlayingContent, resolvePosterUrl } from './src/cgr/widget.js';
import { startNewFilmPolling, stopNewFilmPolling } from './src/cgr/newFilmPolling.js';
...
// and the house is located: browsing the full ~70-cinema list unfiltered
...
gladys.onAction('search_cinemas', async (fields) => {
  ...
  if (results.length === 0) {
    return {
      en: 'No CGR cinema matches this search.',
      fr: 'Aucun cinéma CGR ne correspond à cette recherche.',
    };
  }
...
gladys.onWidgetGetImage(async (imageKey) => {
  ...
  if (!response.ok) {
    throw new Error(`onWidgetGetImage: cgrcinemas.fr HTTP ${response.status} on ${posterUrl}`);
  }
...
logger.info('Starting the CGR integration...');
